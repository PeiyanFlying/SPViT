...
from engine_channel import train_one_epoch, evaluate
from losses import DistillationLoss, DiffPruningLoss, DistillDiffPruningLoss
from samplers import RASampler
import utils
from functools import partial
import torch.nn as nn
from vit import VisionTransformerTeacher
from vit_channel import VisionTransformerDiffPruning, _cfg, checkpoint_filter_fn
...
def get_args_parser():
    ...
    # channel pruning rate
    parser.add_argument('--addl1', action='store_true', default=False, help="whether add L1 regulariation for pruning")
    parser.add_argument('--pruning_ratio', default=0.25, type=float, help="how many parameters to prune")
...
def main(args):
    ...
    if args.arch == 'deit_small':
        ...
        model_channel_prune = VisionTransformerDiffPruning(
            patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
            pruning_loc=PRUNING_LOC, token_ratio=KEEP_RATE, distill=args.distill
            )
        model_path = './logs/dynamic-vit_deit-small/checkpoint_best_num.pth'
        model_path_t = './deit_small_patch16_224-cd65a155.pth'
        checkpoint = torch.load(model_path, map_location="cpu")
        checkpoint_t = torch.load(model_path_t, map_location="cpu")
        ckpt = checkpoint_filter_fn(checkpoint, model)
        ckpt_t = checkpoint_filter_fn(checkpoint_t, model)
        ...
        if args.distill:
            ...
            model_t.load_state_dict(ckpt_t, strict=True)
            model_t.to(device)
        ...
        model_channel_prune = LVViTDiffPruning(
            patch_size=16, embed_dim=384, depth=16, num_heads=6, mlp_ratio=3.,
            p_emb='4_2',skip_lam=2., return_dense=True,mix_token=True,
            pruning_loc=PRUNING_LOC, token_ratio=KEEP_RATE, distill=args.distill
        )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_grad, model_ema, mixup_fn, args.addl1,
            set_training_mode=args.finetune == '' # keep in eval mode during finetuning
        )

        ###### hard prune at the end of training
        if args.addl1 :
            # update l1 parameters
            print('prune on the each epoch and try to save the best one')
            model_channel_prune.load_state_dict(model.state_dict())
            for layer in model_channel_prune.modules():
                if isinstance(layer, PrunedLayer):
                    for param in layer.parameters():
                        gamma = param.data.kthvalue(int(param.data.shape[1]*args.pruning_ratio), dim=1)[0].item()
                        new_zero = torch.zeros_like(param.data)
                        new_one = torch.ones_like(param.data)
                        param.data = torch.where(param.data > gamma, new_one, new_zero)
        ####
        ...
        test_stats = evaluate(data_loader_val, model_channel_prune, device)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        ...
        if max_accuracy == test_stats["acc1"]:
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({
                    'model': model_channel_prune.state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'epoch': epoch,
                    'model_ema': get_state_dict(model_ema),
                    'scaler': loss_scaler.state_dict(),
