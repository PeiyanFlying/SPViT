...
class MultiheadPredictorLG(nn.Module):
    ...
    def forward(self, x, policy):
        ...
        return multihead_score #, represent_token, placeholder_weights, placeholder_score3
...
class VisionTransformerDiffPruning(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 pruning_loc=[3,6,9], token_ratio=[0.7,0.49,0.343], distill=False):
        """
        Args:
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.pruning_loc:
                ...
                if self.training:
                    out_pred_prob.append(hard_keep_decision.reshape(B, -1))
                ...
                else:    # hard_keep_decision 就是图片中的，不带有 placeholder
                    keep_policy = keep_policy_index(hard_keep_decision.reshape(-1))
                    cls_policy = torch.zeros(B, 1, dtype=keep_policy.dtype, device=keep_policy.device)
                    if p_count == 0:
                        rep_policy = torch.Tensor([[keep_policy.size()[1]+1]]).int()
                        now_policy = torch.cat([cls_policy, keep_policy + 1, rep_policy], dim=1)
                    elif p_count == 1:
                        rep_policy1 = torch.Tensor([[keep_policy.size()[1]+1]]).int()
                        rep_policy2 = torch.Tensor([[keep_policy.size()[1]+2]]).int()
                        now_policy = torch.cat([cls_policy, keep_policy + 1, rep_policy1, rep_policy2], dim=1)
                    elif p_count == 2:
                        rep_policy1 = torch.Tensor([[keep_policy.size()[1]+1]]).int()
                        rep_policy2 = torch.Tensor([[keep_policy.size()[1]+2]]).int()
                        rep_policy3 = torch.Tensor([[keep_policy.size()[1]+3]]).int()
                        now_policy = torch.cat([cls_policy, keep_policy + 1, rep_policy1, rep_policy2, rep_policy3], dim=1)
...
def keep_policy_index(hard_keep_decision):
    result = np.where(hard_keep_decision == 1)
    policy = torch.from_numpy(result[0]).reshape(1,-1)
    return policy
