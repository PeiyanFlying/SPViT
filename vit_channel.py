...
class PrunedLayer(nn.Module):
    def __init__(self, unit_num, req_grad=True): #False
        super(PrunedLayer, self).__init__()
        self.params = nn.Parameter(torch.ones(1, unit_num), requires_grad=req_grad)

    def forward(self, x):
        para = self.params.unsqueeze(0).repeat(x.shape[0], x.shape[1], 1) # 对数据维度进行扩充/沿着指定的维度重复tensor
        #print('x.shape, para.shape',x.shape, para.shape) #（256，197，192）
        return torch.mul(x, para) # see size of each x
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.pruned_layer_3 = PrunedLayer(in_features)
        self.fc1 = nn.Linear(in_features, hidden_features)
        self.act = act_layer()
        self.pruned_layer_4 = PrunedLayer(hidden_features)
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.drop = nn.Dropout(drop)

    def forward(self, x):
        x = self.pruned_layer_3(x)
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.pruned_layer_4(x)
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.pruned_layer_1 = PrunedLayer(dim) #token
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.pruned_layer_2 = PrunedLayer(dim)
    ...
    def softmax_with_policy(self, attn, policy, eps=1e-6):
        ...
        attn = attn - max_att # also for stable training
    ...
    def forward(self, x, policy):
        B, N, C = x.shape
        x = self.pruned_layer_1(x)  # prune 1d
...
class PredictorLG(nn.Module):
    ...
    def forward(self, x, policy): # 输出的是每个 token 的概率：保留/抛弃；输入是保留决策
        x = self.in_conv(x)
        B, N, C = x.size()
        local_x = x[:,:, :C//2]
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:
                spatial_x = x[:, 1:]
                pred_score = self.score_predictor[p_count](spatial_x, prev_decision).reshape(B, -1, 2) # 所有 token 和之前的 decision_mask_分布
                if self.training:
