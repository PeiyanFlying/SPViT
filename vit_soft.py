...
class PredictorLG(nn.Module):
    ...
    def forward(self, x, policy):
        ...
        x = self.out_conv(x).reshape(B, -1, 2)
...
class VisionTransformerDiffPruning(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        predictor_list = [PredictorLG(num, embed_dim) for num in range(len(pruning_loc))]
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:
                spatial_x = x[:, 1:]
                pred_score, curent_mask,threshold = self.score_predictor[p_count](spatial_x, prev_decision)
                if self.training:
                    out_pred_prob.append(curent_mask.reshape(B, init_n))
                    cls_policy = torch.ones(B, 1, 1, dtype=curent_mask.dtype, device=curent_mask.device)
                    policy = torch.cat([cls_policy, curent_mask], dim=1)
